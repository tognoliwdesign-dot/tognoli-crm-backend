"""LEXARYS — Routes Clients (schéma DB réel)"""
...
# ── Colonnes réelles de la table clients en DB ────────────────────────────────────────────
CLIENT_COLUMNS = {
...
def _to_db(data: dict) -> dict:
    """Traduit et filtre les champs pour correspondre au schéma DB."""
    result = {}
...
def _to_api(row: dict) -> dict:
    """Ajoute des alias de compatibilité pour le frontend."""
    if not row:
        return row
    r = dict(row)
...
async def create_client(body: ClientCreate, user=Depends(get_current_user)):
    try:
        ...
        raise HTTPException(500, f"Erreur création client: {str(e)}")
